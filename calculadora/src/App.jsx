...
export default function App() {
  ...
  const activeKey = useKeyboardInput(
    ...
    displayValue,
    error,
    showVariableModal || showHistory || showHelpModal || isFlipped // Desabilita captura quando qualquer modal está aberto ou virado
  );
  ...
  // Get the last calculation for Gemini
  const lastCalculation = history.length > 0 ? `${history[0].expression} = ${history[0].result}` : "";
  ...
  return (
    <>
      ...
      <div
        className="flex items-center justify-center min-h-screen font-sans perspective-1000"
        style={{
      ...
      >
        <div className={`relative w-full max-w-md transition-transform duration-700 transform-style-3d ${isFlipped ? "rotate-y-180" : ""}`}>
          
          {/* Front Face - Calculator */}
          <div className="w-full p-6 bg-gray-900/80 backdrop-blur-sm rounded-2xl shadow-2xl border-2 border-purple-500/50 backface-hidden">
            ...
            <h1 
              className="text-3xl font-bold text-center bg-gradient-to-r from-purple-400 via-pink-400 to-purple-400 bg-clip-text text-transparent mb-4 drop-shadow-lg cursor-pointer hover:scale-105 transition-transform"
              onClick={handleFlip}
              title="Clique para falar com o Gemini"
            >
              Calculadora LISP
            </h1>
          ...
          {/* Back Face - Gemini Chat */}
          <div className="absolute top-0 left-0 w-full h-full backface-hidden rotate-y-180">
             <GeminiChat lastCalculation={lastCalculation} onBack={handleFlip} />
          </div>
